...
class BaseMessageMin(MessagesMessage, ABC):
    ...
    async def get_chat_members(self, **kwargs) -> List[MessagesConversationMember]:
        if not self.chat_members:
            self.chat_members = (
                await self.ctx_api.messages.get_conversation_members(peer_id=self.peer_id, **kwargs)
            ).items
